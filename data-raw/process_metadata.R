...
raw_de <- readxl::read_excel(
  "data-raw/Uebersicht der Indikatoren DE.xlsx",
...
raw_en <- readxl::read_excel(
  "data-raw/Uebersicht der Indikatoren EN.xlsx",
...
indicators <- df_de |>
  ...
  dplyr::mutate(
    ...
    Unit_DE = NA_character_, # filled later for all rows after full bind
...
if (file.exists(status_file)) {
  ...
  existing_matches <- which(
    indicators$M_ID %in% api_status$ID[api_status$Active == TRUE]
  )
  ...
  if (length(new_ids) > 0) {
    message(
      "Appending ",
      length(new_ids),
      " new active indicators from API scan..."
    )
    ...
    names(new_rows)[
      names(new_rows) == "Statistische_Grundlagen"
...
# --- 5b. Apply unit extraction across ALL rows (local + API-discovered) ---
extract_unit <- function(name) {
  dplyr::case_when(
    # Percentage / share indicators
    grepl("%", name, fixed = TRUE) ~ "%",
    grepl("(?i)anteil", name, perl = TRUE) ~ "%",
    grepl("(?i)quote", name, perl = TRUE) ~ "%",
    grepl("(?i)anteilswert", name, perl = TRUE) ~ "%",
    # Per-capita
    grepl("je Einwohner", name, ignore.case = TRUE) ~ "je Einwohner",
    grepl("je 1\\.000", name, ignore.case = TRUE) ~ "je 1.000 Einwohner",
    grepl("je 100", name, ignore.case = TRUE) ~ "je 100 Einwohner",
    grepl("je Tsd\\.", name, ignore.case = TRUE) ~ "je 1.000 Einwohner",
    # Money  — more specific (1000 Euro) before generic (Euro) or Millionen
    grepl("1000 Euro", name, ignore.case = TRUE) ~ "1.000 Euro",
    grepl("1\\.000 Euro", name, ignore.case = TRUE) ~ "1.000 Euro",
    grepl("Millionen", name, ignore.case = TRUE) ~ "Mio. Euro",
    grepl("Mio\\. Euro", name, ignore.case = TRUE) ~ "Mio. Euro",
    grepl("in Euro", name, ignore.case = TRUE) ~ "Euro",
    grepl("Euro je", name, ignore.case = TRUE) ~ "Euro",
    # Area
    grepl("qkm", name, ignore.case = TRUE) ~ "qkm",
    grepl(" ha", name, ignore.case = TRUE) ~ "ha",
    grepl("qm", name, ignore.case = TRUE) ~ "qm",
    # Litres
    grepl(" l je", name, ignore.case = TRUE) ~ "l je Einwohner",
    # Absolute counts (Tsd. = thousands)
    grepl("in Tsd\\.", name, ignore.case = TRUE) ~ "Tsd.",
    # Time
    grepl("in Tagen", name, ignore.case = TRUE) ~ "Tage",
    grepl("in Jahren", name, ignore.case = TRUE) ~ "Jahre",
    TRUE ~ NA_character_
  )
}

indicators <- indicators |>
  dplyr::mutate(Unit_DE = extract_unit(Name_DE))

message(
  "Units extracted: ",
  sum(!is.na(indicators$Unit_DE)),
  " of ",
  nrow(indicators),
  " indicators"
)
